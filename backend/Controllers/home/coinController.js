...
class coinController {
    ...
    //         const { id } = req.params;
    //         console.log(id,"\n",form)
    //         const { coinName, coinSymbol, coinAddress, Example } = fields;
            
    //         // Fix: Properly parse Example data with error handling
    //         let parsedCoinExample = [];
    //         try {
    //             // Check if Example exists and is an array
    //             if (Example && Array.isArray(Example)) {
    //                 parsedCoinExample = JSON.parse(Example[1]); // Changed from Example[1] to Example[0]
    //             }
    //         } catch (parseError) {
    ...
    //         if (files.coinImg?.[0] || files.coinBarcode?.[0]) {
    //             // Validate file types
    //             const allowedImageTypes = ['image/jpeg', 'image/png', 'image/webp'];
                
    //             if (files.coinImg?.[0] && !allowedImageTypes.includes(files.coinImg[0].mimetype)) {
    //                 return responseReturn(res, 400, { 
    //                     error: 'Invalid coin image type. Allowed types: JPEG, PNG, WebP' 
    //                 });
    //             }
    
    //             if (files.coinBarcode?.[0] && !allowedImageTypes.includes(files.coinBarcode[0].mimetype)) {
    //                 return responseReturn(res, 400, { 
    //                     error: 'Invalid barcode image type. Allowed types: JPEG, PNG, WebP' 
    //                 });
    //             }
    ...
    //                 // Update the data object with new image URLs if uploads were successful
    //                 if (files.coinImg?.[0]) {
    ...
    //                 // Delete old images only if new ones were uploaded successfully
